...
def main():
    ...
    if args.src_file.endswith(".txt"):
        ...
        for i in tqdm(range(0, len(lines_in))):
            sentence = lines_in[i]
            paraphrases = eda(sentence=sentence)
            out.append({
                "src_text": sentence,
                "tgt_texts": paraphrases
            })
        ...
        origin_data = get_json_data(args.src_file)
        lines_in = [item['sentence'] for item in origin_data]

        for i in tqdm(range(0, len(lines_in))):
            sentence = lines_in[i]
            paraphrases = eda(sentence=sentence)
            origin_data[i]['aug_texts'] = paraphrases


    os.makedirs(os.path.dirname(args.tgt_file), exist_ok=True)
    write_json_data(origin_data, args.tgt_file)
    print(args.src_file, 'has been processed........')
